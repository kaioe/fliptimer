...
import {
    ...
    PRESET_SLIDER_THUMBS_KEY,
    PRESET_TRACK_MAX_KEY,
    FLIPTIMER_COUNTER_PCT_KEY,
    FLIPTIMER_APP_BG_KEY,
    PRESET_SOUND_KINDS,
    SOUNDS_MANIFEST_URL,
    SOUND_UPLOAD_URL,
    SOUND_DELETE_URL,
    ...
    loadCounterSizePct,
    applyCounterSizePct,
    savePreloadedSoundSelectionsToStorage,
...
import {
    ...
    minutesToStartTime,
    normalizePreset,
    formatPresetMinuteLabel,
    loadPresetsFromStorage,
    savePresetsToStorage,
    loadActivePresetIdFromStorage,
    saveActivePresetIdToStorage,
    loadSliderThumbsFromStorage,
    normalizeSliderThumbState,
    getPresetMinuteSliderStep,
    snapPresetMinutesToStep,
    formatPresetMinuteSliderLabel,
    PRESET_SLIDER_OVERLAP_THRESH_PCT,
    PRESET_THUMB_RGB,
    presetSliderCompleted01,
    presetSumRgbToHex,
    presetContrastTextOnRgb,
    saveSliderThumbsToStorage,
    fetchPresetTimersDocument,
    applySoundsFromJsonRoot,
    optimizeImageFileForBackground,
    blobToDataUrlString,
    persistAppBgState,
    applyAppBackgroundState,
    syncPresetJsonToProjectFile,
    presetActionIcon,
    getAppBackgroundDataUrlForSync,
...
export function initPresetTimers(clock, refreshToolbar) {
    ...
    $(".preset-settings-file--bg").on("change", function() {
        var input = this;
    ...
    $settingsFrame.on("click", ".preset-file-drop__btn--bg", function(e) {
        e.preventDefault();
        ...
        var hasFile = !!(stChk && stChk.dataUrl);
        if (hasFile) {
    ...
    $settingsFrame.on("dragover", ".preset-file-drop__zone--bg", function(e) {
        e.preventDefault();
        e.stopPropagation();
        $(this).addClass("preset-file-drop__zone--dragover");
    });
    $settingsFrame.on("dragleave", ".preset-file-drop__zone--bg", function(e) {
    ...
    $settingsFrame.on("drop", ".preset-file-drop__zone--bg", function(e) {
        e.preventDefault();
        e.stopPropagation();
        var zone = this;
    ...
    function uploadSoundFile(file, kind) {
        var fd = new FormData();
        fd.append("file", file);
        $.ajax({
        ...
        .done(function(data) {
            var fileName = data && data.file;
            if (fileName) {
                var sel = loadPreloadedSoundSelectionsFromStorage();
                sel[kind] = fileName;
                savePreloadedSoundSelectionsToStorage(sel);
                syncPresetJsonToProjectFile(presets);
            }
            refreshTimerSoundSettingsUi();
        })
        .fail(function() {
            window.alert("Could not upload sound file.");
        });
    }

    function deleteSoundFile(fileName) {
        $.ajax({
            url: SOUND_DELETE_URL,
            type: "POST",
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify({ file: fileName }),
        })
        .done(function(data) {
            var remaining = data && Array.isArray(data.files) ? data.files : [];
            var sel = loadPreloadedSoundSelectionsFromStorage();
            ...
            savePreloadedSoundSelectionsToStorage(sel);
            syncPresetJsonToProjectFile(presets);
            populatePreloadedSoundSelects(remaining);
        })
        .fail(function() {
            window.alert("Could not delete sound file.");
        });
    }
    ...
    function initTimerSoundSettingsUi() {
        ...
        $settingsFrame.on("click", ".preset-sound-upload-btn", function() {
            var kind = $(this).attr("data-sound-kind");
            if (!kind) { return; }
            var $row = $(this).closest(".preset-settings-upload-row--sound");
            var input = $row.find(".preset-sound-file-input")[0];
            if (!input) { return; }
            $(input).val("");
            $(input).one("change", function() {
                ...
                uploadSoundFile(file, kind);
            });
            input.click();
        });
        $settingsFrame.on("click", ".preset-sound-delete-btn", function() {
            var $sel = $(this).closest(".preset-settings-upload-row--sound").find(".preset-settings-sound-preloaded-select");
            var fileName = $sel.val();
            if (!fileName) { return; }
    ...
    $presetModalHeader.on("touchstart.presetModalPanelDrag", function(e) {
        ...
        if ($(e.target).closest(".preset-counter-size-wrap, .preset-minutes-slider").length) {
            return;
        }
    ...
    // Sound selections always come from fliptimer.json (no localStorage for sounds).
    var stored = loadPresetsFromStorage();
    function afterLoad(doc) {
        if (doc && doc.jsonRoot) {
    ...
    if (stored !== null) {
        ...
        fetchPresetTimersDocument().then(afterLoad);
    } else {
